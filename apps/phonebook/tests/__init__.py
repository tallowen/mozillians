...
def user(**kwargs):
    ...
    if 'email' not in kwargs:
        defaults['email'] = ''.join(random.choice(letters)
                                    for x in xrange(15)) + '@example.com'
    if 'first_name' not in kwargs:
        defaults['first_name'] = ''.join(random.choice(letters)
                                            for x in xrange(15))
    if 'last_name' not in kwargs:
        defaults['last_name'] = ''.join(random.choice(letters)
                                            for x in xrange(15))
...
def add_profilepic(user):
    ...
    with open(os.path.join(os.path.dirname(__file__), 'profile-photo.jpg')) as f:
        r = client.post(reverse('profile.edit'),
            dict(first_name=user.first_name, last_name=user.last_name,
                 photo=f))
